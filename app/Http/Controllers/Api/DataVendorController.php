...
//GET /api/vendor/kategori/{kategoriId}
    ...
    public function storeWithKategori(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id_kategori_vendor' => 'required_without:nama_kategori_baru|integer|exists:kategori_vendors,id',
            'nama_kategori_baru' => 'required_without:id_kategori_vendor|string|max:255',
            'nama_vendor' => 'required|string|max:255',
            'alamat_vendor' => 'required|string',
            'no_telp_vendor' => 'required|string|max:255',
            'deskripsi_vendor' => 'nullable|string'
        ], [
            'id_kategori_vendor.required_without' => 'Pilih kategori yang ada atau masukkan nama kategori baru',
            'id_kategori_vendor.exists' => 'Kategori vendor tidak ditemukan',
            'nama_kategori_baru.required_without' => 'Pilih kategori yang ada atau masukkan nama kategori baru',
            'nama_kategori_baru.max' => 'Nama kategori maksimal 255 karakter',
            'nama_vendor.required' => 'Nama vendor harus diisi',
            'nama_vendor.max' => 'Nama vendor maksimal 255 karakter',
            'alamat_vendor.required' => 'Alamat vendor harus diisi',
            'no_telp_vendor.required' => 'Nomor telepon vendor harus diisi',
        ...
        try {
            ...
            if ($request->has('nama_kategori_baru') && !empty($request->nama_kategori_baru)) {
                $existingKategori = KategoriVendor::where('nama_kategori', $request->nama_kategori_baru)->first();
                
                if ($existingKategori) {
                    $kategoriId = $existingKategori->id;
                    $kategoriInfo = [
                        'status' => 'existing',
                        'message' => 'Menggunakan kategori yang sudah ada',
                        'data' => $existingKategori
                    ];
                } else {
                    $newKategori = KategoriVendor::create([
                ...
                $kategoriId = $request->id_kategori_vendor;
                $kategoriInfo = [
                    'status' => 'selected',
                    'message' => 'Menggunakan kategori yang dipilih'
                ];
            }

            $vendor = DataVendor::create([
                'id_kategori_vendor' => $kategoriId,
                'nama_vendor' => $request->nama_vendor,
                'alamat_vendor' => $request->alamat_vendor,
                'no_telp_vendor' => $request->no_telp_vendor,
                'deskripsi_vendor' => $request->deskripsi_vendor,
